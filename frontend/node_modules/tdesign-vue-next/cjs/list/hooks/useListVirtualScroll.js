...
var useListVirtualScroll = function useListVirtualScroll(scroll, listRef, listItems) {
  ...
  var handleScrollTo = function handleScrollTo(params) {
    ...
    if (!targetIndex && targetIndex !== 0) {
      _common_js_log_log["default"].error("List", "scrollTo: `index` or `key` must exist.");
      return;
    }
    if (targetIndex < 0 || targetIndex >= listItems.value.length) {
      _common_js_log_log["default"].error("List", "".concat(targetIndex, " does not exist in data, check `index` or `key` please."));
      return;
    }
